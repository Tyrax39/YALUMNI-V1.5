...
import { ApiClientError, fetchJson } from "@yalumni/frontend-shared";
...
export function LiveChapterAdmin() {
      ...
      try {
        const responses = await Promise.all(
          chapterTypes.map((communityType) =>
            fetchJson<CommunityListResponse>(
              `/api/backend/api/v1/communities?community_type=${communityType}&membership=all&limit=${chapterPageSize}&offset=0`
            )
          )
        );
        ...
        const chapterMap = new Map<string, CommunityRecord>();
        ...
        setState({
          message:
            caught instanceof ApiClientError
              ? caught.message
              : "Chapter operations could not be loaded.",
          status: "error"
        });
      }
    }
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-5 shadow-soft sm:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h2 className="font-display text-2xl font-semibold text-ink">Chapter operations</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
            This console now reads live country and city chapter inventory from the current
            communities APIs, then links into the existing member-app routes for detailed chapter
            operations.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <StatusBadge label="live API" tone="primary" />
          <StatusBadge label="RBAC gated" tone="neutral" />
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          detail="Country and city chapter records currently loaded."
          icon={<Compass aria-hidden="true" className="h-5 w-5" />}
          label="Chapters loaded"
          value={String(state.chapters.length)}
        />
        <MetricCard
          detail="Combined roster counts visible from the live records."
          icon={<Users aria-hidden="true" className="h-5 w-5" />}
          label="Visible members"
          value={String(totalMembers)}
        />
        <MetricCard
          detail="Country-level chapter entries found in the current dataset."
          icon={<MapPinned aria-hidden="true" className="h-5 w-5" />}
          label="Country chapters"
          value={String(countryChapterCount)}
        />
        <MetricCard
          detail="Deeper aggregation, exports, and governance workflows still remain partial."
      ...
      {state.hasMore ? (
        <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          Showing the first {state.chapters.length} live chapter records. Broader pagination and
          cross-chapter aggregation remain follow-up work.
        </p>
      ) : null}
            ...
            >
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="font-display text-xl font-semibold text-ink">{chapter.name}</h3>
              ...
              <div className="flex flex-col gap-2 lg:min-w-[220px]">
                <ActionLink href={resolveMemberAppUrl(`/communities/${chapter.id}`)} title="Open community detail" />
                <ActionLink
                  href={resolveMemberAppUrl(`/communities/${chapter.id}/dashboard`)}
                  title="Open leader dashboard"
                />
                <ActionLink
                  href={resolveMemberAppUrl(`/admin/chapters/${chapter.id}/analytics`)}
    ...
    <div className="rounded-lg border border-border bg-white p-5 shadow-soft sm:p-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div className="h-28 animate-pulse rounded-lg border border-border bg-surface" key={index} />
        ))}
      </div>
    </div>
  );
...
function resolveMemberAppUrl(pathname: string): string {
  const normalizedPath = pathname.startsWith("/") ? pathname : `/${pathname}`;
  const configuredBaseUrl = process.env.NEXT_PUBLIC_MEMBER_APP_URL?.replace(/\/$/, "");
  ...
  if (hostname === "127.0.0.1" || hostname === "localhost") {
    const memberPort =
      currentPort === 3011 || currentPort === 3012 || currentPort === 3111 || currentPort === 3112
        ? currentPort - 1
        : 3010;
    return `${protocol}//${hostname}:${memberPort}${normalizedPath}`;
  }

  if (hostname.includes("admin")) {
    return `${protocol}//${hostname.replace("admin", "member")}${normalizedPath}`;
  }
