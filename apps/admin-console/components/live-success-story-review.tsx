...
import {
  type SuccessStory,
  type SuccessStoryListResponse,
  approveSuccessStory,
  fetchAdminSuccessStoryQueue,
  rejectSuccessStory,
  requestSuccessStoryChanges
...
type QueueState =
  | { status: "loading" }
  | { response: SuccessStoryListResponse; status: "ready" }
...
export function LiveSuccessStoryReviewQueue() {
  ...
  useEffect(() => {
    ...
    fetchAdminSuccessStoryQueue(query)
      .then((response) => {
        if (isMounted) {
          ...
          setState({
            message:
              caught instanceof Error
                ? caught.message
                : "The success story review queue could not be loaded.",
            status: "error"
          });
        }
      });
  ...
  async function runAction(
    ...
    setBusyAction(`${action}:${story.id}`);
    setMessage(null);

    const payload = { reviewer_note: notes[story.id]?.trim() || null };

    try {
      if (action === "approve") {
        await approveSuccessStory(story.id, payload);
        setMessage("Success story published.");
      } else if (action === "reject") {
        await rejectSuccessStory(story.id, payload);
        setMessage("Success story rejected.");
      } else {
        await requestSuccessStoryChanges(story.id, payload);
      ...
      setMessage(caught instanceof Error ? caught.message : "Success story review action failed.");
    } finally {
      setBusyAction(null);
    }
  }
  ...
  return (
    <section className="rounded-lg border border-border bg-white p-5 shadow-soft sm:p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted">
                Review member-submitted impact stories before they appear in the member library.
              </p>
            </div>
          </div>
        </div>
      ...
      {state.status === "loading" ? <LoadingPanel label="Loading story queue." /> : null}
      {state.status === "error" ? <ErrorPanel message={state.message} /> : null}

      {state.status === "ready" ? (
        <div className="mt-5 grid gap-4">
          {state.response.stories.length ? (
            state.response.stories.map((story) => (
              <StoryReviewRow
                ...
                onAction={runAction}
                onNoteChange={(value) =>
                  setNotes((current) => ({ ...current, [story.id]: value }))
                }
                story={story}
              />
            ))
          ) : (
            <EmptyPanel label="No success stories match this review queue." />
          )}
        </div>
      ) : null}
    </section>
...
function StoryReviewRow({
  ...
  note: string;
  onAction: (story: SuccessStory, action: "approve" | "reject" | "request-changes") => void;
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
        <div>
          ...
          <p className="mt-3 text-xs font-bold uppercase tracking-[0.1em] text-muted">
            {[story.country, story.program, story.impact_metric].filter(Boolean).join(" / ") ||
              "No story metadata"}
          </p>
          <p className="mt-2 text-xs font-semibold text-muted">
            Submitted by {story.created_by_display_name ?? "unknown member"} on{" "}
        ...
        <div className="grid gap-3">
          ...
          <ActionButton
            busy={busyAction === `approve:${story.id}`}
            icon={<CheckCircle2 aria-hidden="true" className="h-4 w-4" />}
            label="Approve"
            onClick={() => onAction(story, "approve")}
            tone="primary"
          />
          <ActionButton
            busy={busyAction === `request-changes:${story.id}`}
            icon={<ClipboardCheck aria-hidden="true" className="h-4 w-4" />}
            label="Request changes"
            onClick={() => onAction(story, "request-changes")}
            tone="neutral"
          />
          <ActionButton
            busy={busyAction === `reject:${story.id}`}
            icon={<XCircle aria-hidden="true" className="h-4 w-4" />}
            label="Reject"
            onClick={() => onAction(story, "reject")}
            tone="danger"
          />
        </div>
      </div>
