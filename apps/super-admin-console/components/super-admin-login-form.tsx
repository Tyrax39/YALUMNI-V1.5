...
async function getCsrfToken(): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error("The super-admin session could not be prepared.");
  }
  ...
  return (
    <form className="rounded-lg border border-border bg-white p-6 shadow-soft" onSubmit={handleSubmit}>
      ...
      <button
        ...
        type="submit"
      >
        {isSubmitting ? "Signing in..." : "Sign in to super-admin console"}
      </button>

      <p className="mt-5 text-center text-sm text-muted">
        Operational admin console is separate at{" "}
        <Link className="font-semibold text-primary hover:underline" href="http://127.0.0.1:3011/login">
          127.0.0.1:3011
        </Link>
        .
      </p>
