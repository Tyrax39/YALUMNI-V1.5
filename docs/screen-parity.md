...
| `chapter_analytics_dashboard` | `/admin/chapters/[chapterId]/analytics` | live route | live API | implemented | Chapter analytics derives live chapter health, operational priorities, moderation footprint, activity trends, governance coverage, and export snapshots from current community, roster, invitation, post, and moderation APIs. |
| `chapter_leader_dashboard` | `/communities/[communityId]/dashboard` | live route | live API | implemented | Dedicated leader dashboard uses live community, roster, invitation, post, report, removed-post, and removed-comment APIs with operational priorities, moderation backlog, leadership coverage, and queue-pressure signals. |
...
| `introduction_requests` | `/messages/introductions` | live route | live API | implemented | Introduction center supports live member search, request creation, incoming accept/decline, outgoing cancellation, direct-message handoffs, and request status tracking. |
| `introduction_requests_desktop` | `/messages/introductions` | responsive reference | live API | implemented | Desktop reference is implemented through the same responsive live introduction route. |
...
| `mobile_onboarding_flow` | `/onboarding` | code-native route | live API | implemented | Mirrors the mobile onboarding export with persisted progress, live profile/affiliation/verification readiness, completion gating, blocker guidance, and next actions. |
...
- Missing backend modules: deeper chapter analytics aggregation/export pipelines, plus advanced election workflows such as nominations, signed/certified audit packets beyond the CSV export foundation, chapter/cohort voter-roll imports, richer candidate nomination states beyond draft approve/reject, dedicated admin detail subroutes beyond the consolidated admin-console panel, and external payment-provider hardening beyond the current local-confirmed contribution flow.
